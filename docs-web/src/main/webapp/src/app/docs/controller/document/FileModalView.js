...
angular.module('docs').controller('FileModalView', function ($uibModalInstance, $scope, $state, $stateParams, $sce, Restangular, $transitions, $http, $window) {
  ...
  Restangular.one('file/list').get({ id: $stateParams.id }).then(function (data) {
   ...
   $scope.getFileContent = function() {
    return $http.get('../api/file/' + $stateParams.fileId + '/data', {
      params: { size: 'content' },
      responseType: 'text' // 假设是文本文件
  ...
  // 调用百度翻译API
  $scope.translateWithBaidu = function(text, from, to) {
    const salt = Date.now();
    const sign = MD5(BAIDU_TRANSLATE_APPID + text + salt + BAIDU_TRANSLATE_KEY);
    
    return $http.jsonp('http://api.fanyi.baidu.com/api/trans/vip/translate', {
      params: {
        q: text,
        appid: BAIDU_TRANSLATE_APPID,
        salt: salt,
        from: from || 'auto', // 自动检测源语言
        to: to || 'zh',       // 默认翻译为中文
        sign: sign
      },
  ...
  $scope.translateFile = function() {
    ...
    $scope.getFileContent().then(function(content) {
      // 2. 调用百度翻译
      return $scope.translateWithBaidu(content);
    }).then(function(response) {
      // 3. 处理翻译结果
      if (response.data && response.data.trans_result) {
        ...
        // 4. 在新窗口中显示结果
        const translationWindow = $window.open('', '_blank');
      ...
      alert('翻译失败: ' + (error.data || error.message));
    }).finally(function() {
